...
public class GameBoardElementsSelector : MonoBehaviour
{
    ...
    private bool IsEnoughAmountSelectedElements => _selectedElements.Count >= 3;
    ...
    public async UniTask<List<Element>> SelectingElements()
    {
        while (true)
        {
            ...
            if (_input.EndClick)
            {
                if (_isSelecting)
                {
                    if (IsEnoughAmountSelectedElements)
                    {
    ...
    private bool TrySelectElement(out Element result)
    {
        ...
        Vector2 mousePosition = _camera.ScreenToWorldPoint(_input.Position);
